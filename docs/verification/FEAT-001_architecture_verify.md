...
| desktop/application/supervisor.go | application | Dependency Direction Violation | APPROVED EXCEPTION | Layer 'application' is not allowed to depend on layer 'delivery' (imported: context) | Đảo ngược phụ thuộc bằng cách định nghĩa interface/port ở lớp 'application'. |
