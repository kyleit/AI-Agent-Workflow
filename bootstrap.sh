...
show_help() {
    ...
    echo ""
    echo "--------------------------------------------------------"
    PYTHONPATH="$FRAMEWORK_ROOT/skills/workflow-runtime" aiwf --help
}
...
case "$COMMAND" in
    ...
    help|-h|--help)
        ...
        ;;
    *)
        PYTHONPATH="$FRAMEWORK_ROOT/skills/workflow-runtime" python3 -m workflow_runtime "$COMMAND" "$@"
        ;;
esac
EOF
