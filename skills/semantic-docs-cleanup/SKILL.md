...
## Hard Rules
- Work only inside `docs/` and `.agents/scratch/semantic-docs-cleanup/` unless the user explicitly asks for rule updates.
...
2. Run a dry-run inventory:
   ```bash
   aiwf cleanup --dry-run
   ```
3. Review the generated dry-run report under `.agents/scratch/semantic-docs-cleanup/reports/`.
4. If classification looks wrong, rerun with explicit overrides:
   ```bash
   aiwf cleanup --dry-run --override docs/path/file.md=workflow-runtime:reports
   ```
5. Apply only after the dry-run has no blocking failures and the user asked for cleanup:
   ```bash
   aiwf cleanup --apply [--feature-family NAME]
   ```
6. Independently validate the result:
   ```bash
   aiwf cleanup --validate-only
   ```
...
## Output
The script writes:
- `.agents/scratch/semantic-docs-cleanup/reports/<timestamp>_inventory.json`
- `.agents/scratch/semantic-docs-cleanup/reports/<timestamp>_plan.json`
- `.agents/scratch/semantic-docs-cleanup/reports/<timestamp>_report.md`
- `.agents/scratch/semantic-docs-cleanup/reports/<timestamp>_review.md`
