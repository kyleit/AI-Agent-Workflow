...
from .benchmark_helpers import _file_size, compute_accuracy, setup_workspace
...
def run_old_init(workspace_dir: str, ctx: dict[str, Any]) -> tuple[float, int, int, dict[str, Any]]:
    """
...
def run_new_init(workspace_dir: str, ctx: dict[str, Any]) -> tuple[float, int, int, dict[str, Any]]:
    """
...
def run_benchmark(n_runs: int = 5) -> list[BenchResult]:
    ...
    for uc in USE_CASES:
        with tempfile.TemporaryDirectory(prefix="aiwf_bench_") as tmp_dir:
            setup_workspace(tmp_dir, cast(dict[str, Any], uc["context"]))

            # Run each flow n_runs times, take median
            old_latencies: list[float] = []
            new_latencies: list[float] = []
            old_bytes_list: list[int] = []
            new_bytes_list: list[int] = []
            old_heavy_list: list[int] = []
            new_heavy_list: list[int] = []
            old_result: dict[str, Any] = {}
            new_result: dict[str, Any] = {}

            ctx_dict = cast(dict[str, Any], uc["context"])

            for _ in range(n_runs):
                lat, br, ho, res = run_old_init(tmp_dir, ctx_dict)
            ...
            for _ in range(n_runs):
                lat, br, ho, res = run_new_init(tmp_dir, ctx_dict)
...
def export_json(results: Any, output_path: str) -> None:
    from workflow_runtime.presentation.cli.benchmark_exporter import (
        export_json as _export_json)
    return _export_json(results, output_path)
