...
class ReportPublisher:
    ...
    def _render_svg_timeline(self, events: List[Dict[str, Any]]) -> str:
        ...
        for i, ev in enumerate(events):
            ...
            svg_str += f'  <text x="{x + 5}" y="{y + 25}" font-family="Arial" font-size="10" fill="#ffffff">{str(ev.get("stage") or "")[:8]}</text>\n'
        svg_str += '</svg>\n'
        return svg_str
    ...
    async def publish_report(self, session_details: Dict[str, Any], path_slug: str) -> Tuple[str, str]:
        ...
        # Retrieve events
        events: list[Any] = cast(list[Any], session_details.get("timeline_events", [])) if isinstance(session_details.get("timeline_events"), list) else []
