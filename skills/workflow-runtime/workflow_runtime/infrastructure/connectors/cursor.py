...
class CursorConnector(ProviderConnector, ITranscriptParser):
    ...
    def compute_fingerprint(self, raw_line: dict[str, Any]) -> str:
        ...
        rdict = raw_line
        usage = cast(dict[str, Any], rdict.get("usage") or rdict.get("tokenCount") or {})
        fields: dict[str, Any] = {
    ...
    def extract_tool_tokens(self, raw_line: dict[str, Any]) -> int:
        usage = cast(dict[str, Any], raw_line.get("usage") or raw_line.get("tokenCount") or {})
    ...
    def parse_with_fingerprint(
        ...
        model = str(rdict.get("model", "cursor-unknown"))
