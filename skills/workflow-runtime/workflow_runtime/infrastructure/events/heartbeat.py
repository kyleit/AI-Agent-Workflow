...
def generate_heartbeat_string(session: dict[str, Any]) -> str:
    conv_id = session.get("conversation_id", "unknown")

    work_item: dict[str, Any] = cast(dict[str, Any], session.get("work_item", {})) if isinstance(session.get("work_item"), dict) else {}
    wi_str = "None"
    if work_item:
    ...
    ctx: dict[str, Any] = cast(dict[str, Any], session.get("context_usage", {})) if isinstance(session.get("context_usage"), dict) else {}
...
def print_heartbeat(session: dict[str, Any]) -> None:
