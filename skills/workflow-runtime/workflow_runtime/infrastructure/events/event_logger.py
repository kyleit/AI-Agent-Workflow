...
from workflow_runtime.infrastructure.filesystem.atomic_writer import (
    append_jsonl, read_jsonl_safe)
from workflow_runtime.infrastructure.session.state_path import get_events_path
...
class EventLogger:
    ...
    def events_path(self) -> str:
        return get_events_path(self.workspace_root)
...
def get_logger(workspace_root: Optional[str] = None) -> EventLogger:
    ...
    with _logger_lock:
        if _default_logger is None or (
            workspace_root and _default_logger.workspace_root != workspace_root
        ):
            _default_logger = EventLogger(workspace_root)
        return _default_logger


def emit_event(event_type: str, payload: dict[str, Any], event_id: Optional[str] = None) -> str:
