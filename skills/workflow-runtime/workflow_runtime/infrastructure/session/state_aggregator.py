...
from workflow_runtime.infrastructure.session.state_path import (
    ensure_dirs, get_dashboard_path, get_legacy_session_path, get_state_file)
...
class StateAggregator:
    ...
    def aggregate(self) -> dict[str, Any]:
        ...
        self._read_substate("recovery", errors)

        if errors:
            health = "degraded"

        # Phase status
        phases: dict[str, Any] = cast(dict[str, Any], runtime.get("phases", {})) if isinstance(runtime.get("phases"), dict) else {}
        ...
        tasks_map: dict[str, Any] = cast(dict[str, Any], raw_tasks) if isinstance(raw_tasks, dict) else {}
        failed_tasks = [str(t) for t, d in tasks_map.items() if isinstance(d, dict) and cast(dict[str, Any], d).get("status") == "failed"]
        _pending_tasks = [str(t) for t, d in tasks_map.items() if isinstance(d, dict) and cast(dict[str, Any], d).get("status") == "pending"]

        # Worker and lock counts
        active_workers: dict[str, Any] = cast(dict[str, Any], runtime.get("active_workers", {})) if isinstance(runtime.get("active_workers"), dict) else {}
        worker_count = sum(
            1 for w in active_workers.values()
            if w.get("status") in ("running", "starting")
        )
        active_locks: dict[str, Any] = cast(dict[str, Any], runtime.get("active_locks", {})) if isinstance(runtime.get("active_locks"), dict) else {}
    ...
    def write_legacy_snapshot(self, dashboard: Optional[dict[str, Any]] = None) -> str:
        """
        ...
        existing = cast(dict[str, Any], read_json_safe(legacy_path) or {})
        ...
        snapshot.update({
            ...
            "current_skill": dash.get("current_skill", ""),
            "suggested_next_skill": dash.get("suggested_next_skill", ""),
            "checkpoint": dash.get("checkpoint", 0),
            "release_allowed": dash.get("release_allowed", False),
            "debug_allowed": dash.get("debug_allowed", False),
            "verify_allowed": dash.get("verify_allowed", False),
            "implementation_status": dash.get("implementation_status", ""),

            # Deprecation markers
            "_deprecated": True,
            "_generated": True,
            "_source": "dashboard.json",
            "_generated_at": dash.get("_generated_at", ""),
            "_health": dash.get("_health", "healthy"),
        })
    ...
    def _read_substate(self, category: str, errors: list[str]) -> dict[str, Any]:
    ...
    def _compute_gates(
        ...
        if phases and not all_phases_complete:
            phases_map = phases
            incomplete: list[str] = [
                str(pid) for pid, pd in phases_map.items()
                if isinstance(pd, dict) and cast(dict[str, Any], pd).get("status") != "completed"
            ]
...
def load_dashboard(workspace_root: Optional[str] = None) -> dict[str, Any]:
