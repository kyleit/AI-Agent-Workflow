...
class ImplementationLedger:
    ...
    def init_from_blueprint(self, blueprint_json: dict[str, Any]) -> dict[str, Any]:
        """
        ...
        if bp_phases:
            ...
            phase_map: OrderedDict[str, list[str]] = OrderedDict()
            for pkg in impl_packages:
                module = str(pkg.get("module", "Phase 1"))
    ...
    def get_phase_summary(self) -> list[dict[str, Any]]:
        ...
        phases_raw = ledger.get("phases", [])
        phases = cast(list[dict[str, Any]], phases_raw) if isinstance(phases_raw, list) else []
        for phase in phases:
            summary.append({
    ...
    def get_failed_tasks(self) -> list[str]:
        ...
        if not isinstance(tasks, dict):
            return []
        failed: list[str] = []
        tasks_dict = cast(dict[str, dict[str, Any]], tasks)
        for tid, td in tasks_dict.items():
            if td and td.get("status") == TASK_STATUS_FAILED:
    ...
    def _parse_phases(
        self,
        bp_phases: list[dict[str, Any]],
        impl_packages: list[dict[str, Any]],
    ) -> list[dict[str, Any]]:
        ...
        for bp_phase in bp_phases:
            ...
            phase_name = str(bp_phase.get("phase_name", bp_phase.get("id", "")))
            raw_tasks_obj = bp_phase.get("tasks", [])
            raw_tasks = cast(list[Any], raw_tasks_obj) if isinstance(raw_tasks_obj, list) else []
            task_ids: list[str] = [str(x) for x in raw_tasks]
            if not task_ids:
    ...
    def _task_in_phase(self, pkg: dict[str, Any], phase_name: str) -> bool:
    ...
    def _get_phase(self, ledger: dict[str, Any], phase_id: str) -> dict[str, Any]:
        """Find a phase in ledger by ID. Raises ValueError if not found."""
        phases_raw = ledger.get("phases", [])
        phases_list = cast(list[dict[str, Any]], phases_raw) if isinstance(phases_raw, list) else []
        for phase in phases_list:
            if phase.get("id") == phase_id:
                return phase
        raise ValueError(f"Phase '{phase_id}' not found in ledger.")

    def _find_phase_for_task(self, task_id: str, phases: list[dict[str, Any]]) -> str:
        """Find the phase ID that contains a given task_id."""
        for phase in phases:
            tasks = phase.get("tasks", [])
            if isinstance(tasks, list) and task_id in tasks:
                return str(phase.get("id", "Phase 1"))
        return "Phase 1"
