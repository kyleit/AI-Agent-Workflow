...
DEFAULT_CONFIG = {
    "architecture": {
        ...
        "composition_roots": ["cmd", "main.go", "desktop/main.go", "workflow_runtime.__main__"],
        "exceptions": [
            {
...
def load_architecture_config(root_dir: str = ".") -> dict[str, Any]:
...
def get_file_layer(filepath: str, config: dict[str, Any]) -> Optional[str]:
    ...
    for layer_name, layer_cfg in layers.items():
        paths: list[Any] = cast(list[Any], layer_cfg.get("paths", [])) if isinstance(layer_cfg.get("paths"), list) else []
        for p in paths:
...
def run_architecture_validation(root_dir: str = ".") -> Tuple[bool, int, list[dict[str, Any]], dict[str, Any]]:
    ...
    exceptions_list: list[dict[str, Any]] = cast(list[dict[str, Any]], config["architecture"].get("exceptions", []))
    ...
    for filepath in files_to_check:
        ...
        for imp in imports:
            ...
            imp_layer = None
            for other_file_raw, other_info_raw in dependency_graph.items():
                other_file = str(other_file_raw)
                other_info = cast(dict[str, Any], other_info_raw)
                ...
                if imp == other_mod or other_mod.endswith("." + imp) or (len(mod_parts) > 0 and mod_parts[-1] == imp):
                    imp_layer = cast(Optional[str], other_info.get("layer"))
                    break
...
def generate_architecture_report(work_item_id: str, passed: bool, score: int, violations: list[dict[str, Any]], dep_graph: dict[str, Any]) -> str:
...
| :--- | :---: | :---: |
| **Dependency Direction** | 25 | {"PASS" if not any(v.get("type") == "Dependency Direction Violation" for v in violations) else "FAIL"} |
| **Domain Purity** | 20 | {"PASS" if not any(v.get("type") == "Domain Purity Violation" for v in violations) else "FAIL"} |
| **Application Boundary** | 15 | PASS |
| **Infrastructure Isolation** | 15 | PASS |
| **Delivery Boundary** | 10 | {"PASS" if not any(v.get("type") == "Delivery Bypass Violation" for v in violations) else "FAIL"} |
