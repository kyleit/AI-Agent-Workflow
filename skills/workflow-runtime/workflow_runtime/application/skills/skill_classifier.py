...
def classify_intent(request: str) -> dict[str, Any]:
    req_lower = request.lower()

    workflow_test_keywords = [
        "test aiwf",
        "test workflow",
        "workflow test",
        "framework test",
    ...
    if any(kw in req_lower for kw in workflow_test_keywords):
        return {
            "status": "success",
            "recommended_skill": "brainstorming",
            "recommended_command": "brainstorm",
            "options": [],
            "reason": "Yêu cầu kiểm tra luồng AIWF/tài liệu phải dùng standard workflow: roadmap -> architecture-review -> brainstorming -> architecture-review -> plan -> architecture-review -> blueprint -> architecture-review -> approval."
        }
