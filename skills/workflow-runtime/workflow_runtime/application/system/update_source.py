...
class SourceRepositoryService:
    ...
    def check_status(self, auto_init: bool = True, repo_url: str = DEFAULT_REPO_URL, releases_only: bool = True) -> dict[str, Any]:
    ...
    def pull_ff(self, releases_only: bool = True) -> bool:
        if releases_only:
            ...
            if latest_tag:
                ...
                code, _, err = self._run_git(["checkout", f"tags/{latest_tag}"])
                if code == 0:
                    return True
                print(f"Warning: Checkout tag {latest_tag} failed: {err}. Falling back to branch pull.", file=sys.stderr)

        code, _, err = self._run_git(["pull", "--ff-only", self.remote, self.branch])
        if code != 0:
...
def handle_update_source(args: Any) -> int:
    source_path = str(getattr(args, "source", None) or getattr(args, "source_opt", None) or os.environ.get("AIWF_SOURCE_PATH") or ".")
    remote = str(getattr(args, "remote", None) or "origin")
    branch = str(getattr(args, "branch", None) or "main")
    url = str(getattr(args, "url", None) or DEFAULT_REPO_URL)
    releases_only = not bool(getattr(args, "allow_prerelease", False))
    ...
    print(f"Local commit: {status.get('commit')}")
    ...
    if status.get("is_dirty") and not getattr(args, "allow_dirty", False):
