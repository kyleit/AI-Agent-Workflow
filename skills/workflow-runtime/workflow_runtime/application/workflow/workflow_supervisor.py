...
from workflow_runtime.application.agent.agent_dispatcher import AgentDispatcher
from workflow_runtime.application.skills.skill_router import SkillRouter
from workflow_runtime.application.verification.evidence_gate_engine import \
    EvidenceGateEngine
from workflow_runtime.application.workflow.workflow_state_machine import \
...
class WorkflowSupervisor:
    ...
    def run_supervisor_step(self, evidence: Dict[str, Any]) -> str:
        ...
        from workflow_runtime.application.docs.artifact_governance import \
            ArtifactGovernance
        ...
        phase_evidence_files: list[Any] = cast(list[Any], phase_config.get("evidence", [])) if isinstance(phase_config.get("evidence"), list) else []
