...
def load_code_size_policy(root_dir: str = ".") -> dict[str, Any]:
...
def analyze_python_file(filepath: str, policy: dict[str, Any]) -> dict[str, Any]:
    """Phân tích AST kích thước code cho tệp Python."""
    funcs_list: list[dict[str, Any]] = []
...
def analyze_go_file(filepath: str, policy: dict[str, Any]) -> dict[str, Any]:
    """Phân tích kích thước code cho tệp Go."""
    go_funcs: list[dict[str, Any]] = []
    go_classes: list[dict[str, Any]] = []
    metrics: dict[str, Any] = {
        "file_lines": 0,
        "classes": go_classes,
        "functions": go_funcs
...
def run_code_size_audit(root_dir: str = ".") -> tuple[bool, list[dict[str, Any]], dict[str, Any]]:
    ...
    largest_functions: list[dict[str, Any]] = []
    largest_classes: list[dict[str, Any]] = []
    ...
    for filepath, rel_path in files_to_check:
        if filepath.endswith(".py"):
            metrics: dict[str, Any] = analyze_python_file(filepath, policy)
        else:
            metrics: dict[str, Any] = analyze_go_file(filepath, policy)

        file_lines = int(metrics.get("file_lines", 0))
        largest_files.append({"file": rel_path, "lines": file_lines})

        # 1. Kiểm tra giới hạn File lines
        max_file_lines = int(policy.get("max_lines_per_file", 500))
        warn_file_lines = int(policy.get("warning_lines_per_file", 400))
        ...
        max_func_lines = int(policy.get("max_lines_per_function", 60))
        warn_func_lines = int(policy.get("warning_lines_per_function", 45))

        funcs: list[dict[str, Any]] = cast(list[dict[str, Any]], metrics.get("functions", []))
        for func in funcs:
            f_name = str(func.get("name", ""))
            f_lines = int(func.get("lines", 0))
        ...
        max_class_lines = int(policy.get("max_class_lines", 300))
        max_methods = int(policy.get("max_methods_per_class", 20))

        classes_list: list[dict[str, Any]] = cast(list[dict[str, Any]], metrics.get("classes", []))
        for cls in classes_list:
            c_name = str(cls.get("name", ""))
            c_lines = int(cls.get("lines", 0))
            c_methods = int(cls.get("methods_count", 0))
            largest_classes.append({
                "file": rel_path,
                "name": c_name,
                "lines": c_lines,
                "methods_count": c_methods
            })

            if c_lines > max_class_lines:
                violations.append({
                    "file": rel_path,
                    "scope": "Class",
                    "name": c_name,
                    "current_lines": c_lines,
                    "limit": max_class_lines,
                    "status": "APPROVED EXCEPTION" if excepted else "FAIL",
                    "recommendation": f"Tách lớp '{cls['name']}' thành các lớp con chuyên trách (SRP)."
                })
            if c_methods > max_methods:
                violations.append({
                    "file": rel_path,
                    "scope": "ClassMethods",
                    "name": c_name,
                    "current_lines": c_methods,
                    "limit": max_methods,
                    "status": "APPROVED EXCEPTION" if excepted else "FAIL",
    ...
    # Sắp xếp top metrics
    all_metrics["largest_files"] = sorted(largest_files, key=lambda x: int(x.get("lines", 0)), reverse=True)[:10]
    all_metrics["largest_functions"] = sorted(largest_functions, key=lambda x: int(x.get("lines", 0)), reverse=True)[:10]
    all_metrics["largest_classes"] = sorted(largest_classes, key=lambda x: int(x.get("lines", 0)), reverse=True)[:10]
...
def generate_code_size_report(work_item_id: str, passed: bool, violations: list[dict[str, Any]], metrics: dict[str, Any]) -> str:
