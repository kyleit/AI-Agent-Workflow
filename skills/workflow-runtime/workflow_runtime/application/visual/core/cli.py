...
class CLIRunner:
    ...
    def main(self, args: List[str]) -> int:
        ...
        if hasattr(parsed_args, 'command') and parsed_args.command == 'agent':
            from workflow_runtime.application.visual.core.orchestrator import \
            ...
            print(f"[VIR Agent] Goal: {goal}, Max Steps: {max_steps}")
            orch = LifecycleOrchestrator()
            result = orch.execute_run(url=url, profile_name="cli-agent")
            report_dict = {"passed": result == "PASS"}
