...
class TestImpactResolver:
    ...
    def resolve_affected_tests(self, changed_files: list[str]) -> list[str]:
        ...
        knowledge_runtime_root = os.path.join(self.workspace_root, "skills", "knowledge-runtime", "scripts", "knowledge_runtime")
        if os.path.exists(knowledge_runtime_root):
            affected.add("skills/knowledge-runtime/tests/smoke/test_runtime_api.py")

        filtered_changes: list[str] = []
        ...
        if all(any(f_item.endswith(ext) for ext in doc_extensions) for f_item in filtered_changes):
            return list(affected)

        for norm_f in filtered_changes:
            name = os.path.basename(norm_f)

            if name.startswith("test_") and name.endswith(".py") and "/tests/" in norm_f:
                affected.add(norm_f)
                continue

            if name in SOURCE_TO_TEST_MAP:
                mapped_tests = SOURCE_TO_TEST_MAP[name]
...
def validate_test_architecture(workspace_root: str = ".") -> dict[str, Any]:
    errors: list[str] = []
    ...
    found_test_basenames: set[str] = set()
