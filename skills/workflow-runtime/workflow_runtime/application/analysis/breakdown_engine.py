...
def generate_breakdown(session_data: dict[str, Any], workspace_root: str = ".") -> dict[str, Any]:
    ...
    def sum_doc_tokens(folder: str, pattern: str | None = None) -> tuple[int, int, str, list[dict[str, Any]]]:
            ...
            try:
                ...
                with open(log_path, "r", encoding="utf-8") as f:
                    for line_str in f:
                        ...
                        tc_list: list[dict[str, Any]] = cast(list[dict[str, Any]], line.get("tool_calls", [])) if isinstance(line.get("tool_calls"), list) else []
                        if not tc_list and isinstance(content, dict):
                            c_dict = cast(dict[str, Any], content)
                            tc_list = cast(list[dict[str, Any]], c_dict.get("tool_calls", [])) if isinstance(c_dict.get("tool_calls"), list) else []

                        if tc_list:
                            for tc in tc_list:
                                name = tc.get("name")
                                args: dict[str, Any] = cast(dict[str, Any], tc.get("args", {})) if isinstance(tc.get("args"), dict) else {}
                                path_val = None
                                if name == "view_file":
                        ...
                        c_str = str(line.get("content", ""))
                        if type_ == "USER_INPUT":
                            hist_tokens += int(len(c_str) / 3)
                            hist_loads += 1
    ...
    for name, stats in categories.items():
        ...
        dets: list[Any] = cast(list[Any], stats.get("details", [])) if isinstance(stats.get("details"), list) else []
        dets.sort(key=lambda x: cast(int, cast(dict[str, Any], x).get("tokens", 0)), reverse=True)
    ...
    breakdown_list.sort(key=lambda x: cast(int, x.get("tokens", 0)), reverse=True)
...
def update_breakdown_file(session_data: dict[str, Any], workspace_root: str = ".") -> dict[str, Any]:
