...
def get_or_create_cache(source_path: str) -> dict[str, Any]:
...
def build_context_bundle(conversation_id: str, original_tokens: int) -> dict[str, Any]:
    init_rebuilder_tables()
...
def get_rebuild_history(conversation_id: str) -> list[dict[str, Any]]:
    init_rebuilder_tables()
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
