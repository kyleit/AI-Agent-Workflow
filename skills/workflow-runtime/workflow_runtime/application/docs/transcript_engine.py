...
class IncrementalTranscriptReader:
    ...
    def __init__(self, db_conn: sqlite3.Connection | None = None) -> None:
        ...
        """
        self._db = db_conn
        self._memory_cursors: dict[str, Any] = {}  # fallback when no db_conn

    # ------------------------------------------------------------------ #
    #  Public API                                                          #
    # ------------------------------------------------------------------ #

    def read_new_lines(self, file_path: str) -> list[tuple[dict[str, Any], int]]:
        """
    ...
    def _stream_from(self, file_path: str, byte_pos: int) -> list[tuple[dict[str, Any], int]]:
            ...
            while True:
                ...
                try:
                    ...
                    if isinstance(parsed, dict):
                        lines.append((cast(dict[str, Any], parsed), start_offset))
    ...
    def _load_cursor(self, file_path: str) -> tuple[int, str]:
        ...
        if isinstance(entry, dict):
            ...
            return int(str(entry_dict.get("byte_pos", 0))), str(entry_dict.get("file_hash", ""))
        return 0, ""
...
__all__ = [
    ...
    "ensure_transcript_cursors_table",
]
