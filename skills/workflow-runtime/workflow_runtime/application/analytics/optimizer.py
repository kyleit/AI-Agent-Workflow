...
def set_active_policy(policy_name: str) -> dict[str, Any]:
    init_optimizer_tables()
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
...
def calculate_roi(conversation_id: str) -> dict[str, Any]:
    init_optimizer_tables()
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
...
def generate_benchmark_report(original_tokens: int, original_cost: float) -> dict[str, Any]:
    init_optimizer_tables()
...
def get_optimization_leaderboard() -> list[dict[str, Any]]:
    init_optimizer_tables()
