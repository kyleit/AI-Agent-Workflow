...
def _build_registry() -> Any:
    ...
    return cast(Any, InfrastructureLocator).build_default_registry()


def _get_cost_engine():
    """Return module-level CostEngine singleton."""
    from workflow_runtime.application.analytics.cost_engine import cost_engine
    return cost_engine
...
def cmd_parse(args: Any) -> int:
    ...
    sources = {str(r.get("accuracy_source", "unknown")) for r in enriched}
...
def cmd_reprice(args: Any) -> int:
    """Reprice all requests that do not have locked pricing version."""
#     from workflow_runtime.infrastructure.persistence.db import get_project_db_path, init_db_schema
    import sqlite3
    from datetime import datetime

    from workflow_runtime.application.analytics.cost_engine import CostEngine

    conn = sqlite3.connect(cast(Any, InfrastructureLocator).get_project_db_path())
    try:
        cast(Any, InfrastructureLocator).init_db_schema(conn)
        cursor = conn.cursor()
        cursor.execute("""
        ...
        pricing_data = getattr(engine, '_load')()
        ...
        for row in rows:
            ...
            tot_tok = (inp or 0) + (out or 0) + (cache or 0) + (think or 0) + (tool or 0)
...
def cmd_usage(args: Any) -> int:
    """Delegate to usage_validator module."""
    from workflow_runtime.application.verification.usage_validator import \
        UsageValidator
#     from workflow_runtime.infrastructure.persistence.db import get_project_db_path
    import sqlite3
    ...
    try:
        ...
        if args.usage_subcommand == "validate":
            ...
            if res.get("status") == "error":
                sys.stderr.write(str(res.get("message") or "") + "\n")
                return 2
            print(json.dumps(res, ensure_ascii=False))
            return 0
    except Exception as e:
