...
try:
    ...
    sys.modules["aiwf_registry"] = aiwf_registry
...
def test_doctor_registry_backfills_legacy_project_without_aiwf_version(temp_registry_dir, tmp_path):
    """Legacy registry rows without aiwf_version must not crash doctor."""
    project_dir = tmp_path / "legacy-project"
    project_dir.mkdir()
    agents_dir = project_dir / ".agents"
    agents_dir.mkdir()
    (agents_dir / "AI_RULES.md").write_text("# Rules")
    (agents_dir / "skills").mkdir()
    (agents_dir / "MANIFEST.json").write_text('{"version":"6.20.31"}', encoding="utf-8")

    aiwf_registry.save_registry_atomic({
        "schema_version": 1,
        "updated_at": "legacy",
        "projects": [
            {
    ...
    assert report["details"][0]["aiwf_version"] == "6.20.31"
    registry = aiwf_registry.load_registry()
    assert registry["projects"][0]["aiwf_version"] == "6.20.31"
    assert registry["projects"][0]["id"]
