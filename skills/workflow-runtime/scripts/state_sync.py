...
class StateFileLock:
    def __init__(self, workspace_root: str, timeout: float = 5.0, expire_seconds: float = 10.0):
    ...
    def acquire(self) -> bool:
        if not hasattr(_local_state, 'depth'):
            _local_state.depth = 0
            
        if _local_state.depth > 0:
        ...
        while time.time() - start_time < self.timeout:
            if os.path.exists(self.lock_file):
                ...
                try:
                    ...
                    # If this process already owns the lock on disk
                    if lock_pid == pid:
                        _local_state.depth = 1
                        return True
                    
                    if time.time() - locked_at > self.expire_seconds:
                        self._force_release()
                        continue
    ...
    def release(self) -> None:
        if not hasattr(_local_state, 'depth') or _local_state.depth == 0:
            return
        _local_state.depth -= 1
        if _local_state.depth == 0:
            self._force_release()
    ...
    def __enter__(self):
        if not self.acquire():
            raise RuntimeError("Failed to acquire state.lock within timeout")
        return self
...
def aggregate_state(workspace_root: str) -> dict[str, Any]:
    with StateFileLock(workspace_root):
...
def deconstruct_state(workspace_root: str, session: dict[str, Any]) -> None:
    with StateFileLock(workspace_root):
        ...
        def write_if_changed(file_path: str, new_data: dict[str, Any]) -> None:
            old_data = read_json_safe(file_path)
            if old_data != new_data:
                write_json_atomic(file_path, new_data)
        
        write_if_changed(os.path.join(state_dir, "context.json"), context)
        write_if_changed(os.path.join(state_dir, "workflow.json"), workflow)
        write_if_changed(os.path.join(state_dir, "runtime.json"), runtime)
        write_if_changed(os.path.join(state_dir, "approvals.json"), approvals)
        write_if_changed(os.path.join(state_dir, "usage.json"), usage)
        write_if_changed(os.path.join(state_dir, "agents.json"), agents)
