...
- Invoke the canonical runtime via `aiwf vir` instead of embedding runtime execution code directly.
...
- Capture page observation:
  `aiwf vir run --url "http://localhost:3000"`
- Run autonomous agent loop:
  `aiwf vir agent --url "http://localhost:3000" --goal "Verify UI layout"`
- Run Quality Gate check:
  `aiwf vir check --baseline "comp_1" --current "obs_1"`
