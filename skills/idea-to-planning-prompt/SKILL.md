...
# Skill: idea-to-planning-prompt

---

## ⚠ MANDATORY FIRST ACTION — DO THIS BEFORE ANYTHING ELSE

**When this Skill is invoked, the VERY FIRST thing you must do is output this block:**
...
>
> Everything written after `/idea-to-planning-prompt` is a **REQUIREMENT INPUT**.
...
> - Modify source code (`.go`, `.js`, `.ts`, `.svelte`, `.py`, `.sql`, or any other extension)
> - Edit project files outside `docs/brainstorm/`
> - Create implementation files
> - Create test files
> - Update Project Memory
> - Update CHANGELOG
> - Run Git commands (commit, push, branch)
> - Execute or auto-invoke another Skill
> - Continue automatically into Planning, Blueprint, or Implementation
>
> **The ONLY file this Skill may write:**
> ```
> docs/brainstorm/FEAT-XXX_feature_name.md
> ```
> And only after the user explicitly types **Y** at Step 14.
>
...
> [!CAUTION]
> **MANDATORY STOP POINTS — Cannot be bypassed:**
>
> → STOP after displaying Feature Decomposition options (wait for user choice)
> → STOP after asking Clarification Questions (wait for user answers)
> → STOP after displaying "Continue generating Brainstorming document? [Y/N]" (wait for Y or N)
> → STOP after writing the Brainstorming document
>
> **NEVER CONTINUE INTO:** Planning · Blueprint · Implementation · Release · Memory Update
>
> The next Skill **must always be invoked manually** by the user.
> Do not suggest, hint at, or imply that the user should invoke another Skill.
...
```text
/idea-to-planning-prompt <requirements as free text>

Examples:
/idea-to-planning-prompt Add local cache for playwright static files.
/idea-to-planning-prompt Fix login bug.
/idea-to-planning-prompt Bulk insert support. CSV validation. Issue generation.
/idea-to-planning-prompt The DB saves multiline proxy input as a single row.
```
...
Output the following block verbatim as your very first response.
...
### Purpose
Complete, self-contained prompt for the `planning-prompt-to-plan` Skill.
...
> ⚠ The next Skill is `planning-prompt-to-plan`.
...
Self-Validation Checklist
─────────────────────────────────────────────────────────────────────
[ ] I output the DISCOVERY MODE ACTIVE declaration as my first action.
[ ] I did NOT modify any source code files.
[ ] I did NOT edit any project files outside docs/brainstorm/.
[ ] I treated all user input as requirements, not implementation commands.
[ ] I calculated the Requirement Readiness Score.
[ ] I asked clarification questions when score < 85 and stopped.
[ ] I generated 2–3 significantly different solution options.
[ ] I recommended one option with detailed architectural reasoning.
[ ] I asked "Continue generating Brainstorming document? [Y/N]" and stopped.
[ ] I waited for explicit Y before writing any file.
[ ] I stopped after completing Brainstorming generation.
[ ] I did NOT invoke or suggest invoking another Skill automatically.
─────────────────────────────────────────────────────────────────────
...
Status:
  [ Paused — Awaiting feature selection     ]
  [ Paused — Awaiting clarification answers ]
  [ Paused — Awaiting user confirmation Y/N ]
  [ Completed                               ]

Active Feature(s):
  [FEAT-XXX: Name | Feature 1, 2, 3 — pending user selection]
...
Brainstorming File(s) Written:
  [docs/brainstorm/FEAT-XXX_feature_name.md | None]

Self-Validation:
  [ALL PASS | FAILED: item list]

════════════════════════════════════════════════════════
Workflow Paused. Skill responsibility is complete.
The next Skill (planning-prompt-to-plan) must be invoked manually.
════════════════════════════════════════════════════════
```
