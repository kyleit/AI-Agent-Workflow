...
Upon successful environment configuration and verification, output the runtime heartbeat.
...
- **Approval Gate Policy** (Section 1) - Ask user before creating config files, directories, or running unsafe installs.
- **Git Workflow Policy** (Section 2) - Read-only check git repository status; do not commit/push.

**ALLOWED**:
- Run detection and tool version commands.
- Install safe CLI tools and npm/python packages.
- Create `.agents/memory.config.json` after approval.
- Create `.agents/` and `.agents/skills/` directories after approval.
- Print health summaries and recommendations.

**FORBIDDEN**:
- Modify project source code or tests.
- Perform Git commits, tags, or pushes.
- Install unsafe components (Docker, Qdrant, Ollama) or system daemons without explicit user confirmation.
- Auto-run `project-memory-bootstrap`.
