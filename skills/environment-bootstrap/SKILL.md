...
# Linux — Debian/Ubuntu
uname -s && uname -m  # confirm Linux host and architecture
...
All approval or choice prompts in this phase MUST use native Agent/IDE `ask_question` first with the same question, options, and safe default shown below. The `aiwf prompt select` commands are fallback bridge examples only, used when native `ask_question` is unavailable or unsupported. If the fallback returns `PROMPT_UNAVAILABLE`, no user selection occurred; stop or retry through native `ask_question` if it becomes available.
...
Register-ScheduledTask -TaskName "QdrantService" `
  ...
  -Action (New-ScheduledTaskAction -Execute "<QDRANT_BINARY>" -WorkingDirectory "<QDRANT_WORKDIR>") `
  -Principal (New-ScheduledTaskPrincipal -UserId "SYSTEM" -LogonType ServiceAccount)
```

##### Linux (systemd)
1. Copy the `qdrant` binary to a directory available on `PATH`.
2. Create a systemd service file in the system unit directory with:
```ini
[Unit]
