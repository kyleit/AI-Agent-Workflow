...
entrypoint_role: LEGACY_WORKFLOW_ALIAS
...
## 🏛️ Entrypoint & Authority Boundary

The `software-development-workflow` skill acts as a **Legacy Workflow Alias / Utility Map**. It outlines the SDLC lifecycle mapping, but **delegates canonical workflow entry, intake classification, and phase routing authority to `workflow-coordinator`**. It MUST NOT maintain independent entrypoint authority.
...
- **Progress Tracking**: Update status and log progress using `aiwf` when integrated in a workflow session.
...
Feature-Centric SDLC (Enforcing Blueprint-Driven Development)
  ...
  Option 1: Standard Workflow (Medium & Large Features)
    ...
    [Optional] ADR       ──> docs/adr/ADR-XXX_<short_title>.md
                             Skill: create-adr
    4. Design Approval   ──> Native Agent/IDE `ask_question`; runtime `prompt select` fallback bridge only
...
- **Physical Repository Write Policy** (Section 33) — All implementation MUST be physical writes to project filesystem. IDE implementation mode is FORBIDDEN.
- **Global Multi-Language Strict Engineering Policy** (Section 32) — Enforce 3-layer policy model for all managed code.
...
1. If `active_workflow` has `waiting_for` set, and the user's prompt is a continue phrase (e.g., "continue", "tiếp", "tiếp đi", "proceed", "go"):
   ...
   * Run the CLI to inspect and resume the active workflow:
     ```bash
     # Check current workflow status
     aiwf workflow status
     # Then resume the skill indicated by suggested_next_skill in session
     aiwf start --skill <suggested_next_skill> --command <suggested_next_command> --step "Resuming workflow"
     ```
...
Create a native Agent/IDE `ask_question` selection gate first. Only when native `ask_question` is unavailable, use the runtime prompt bridge:
```bash
