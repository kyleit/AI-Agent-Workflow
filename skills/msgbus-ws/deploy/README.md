...
## 0. Prerequisites
- Image pushed to `registry.example.invalid/example/message-bus`.
...
docker push registry.example.invalid/example/message-bus:1.0.0
```
...
helm upgrade --install msgbus-ws deploy/helm/msgbus-ws \
  ...
  --set ingress.host=msgbus.example.invalid \
...
curl -s https://msgbus.example.invalid/health          # {"ok":true,...}
```

## 5. Connect a client (over the domain, TLS)
```bash
export MSGBUS_HOST=msgbus.example.invalid MSGBUS_TLS=1 MSGBUS_TOKEN=<the-token>
...
| Value | Meaning |
| :-- | :-- |
| `image.repository` / `image.tag` | image (default `registry.example.invalid/example/message-bus`) |
