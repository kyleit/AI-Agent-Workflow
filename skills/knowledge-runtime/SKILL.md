...
## 5. Runtime Commands
- `aiwf search --query '<query>' [--top-k N]`
- `aiwf provider list`
- `aiwf provider sync [--name <name>]`
- `aiwf provider config [--name <name>]`
- `aiwf provider status [--name <name>]`
...
## 8. Usage Examples
```python
from workflow_runtime.application.knowledge.knowledge_api import search, read, sync
# Thực hiện tìm kiếm
