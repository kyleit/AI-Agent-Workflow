...
- **500-Line Split Organization**: When a Go file must be split, group extracted files inside one family package directory and keep a single package-level facade/aggregate entry for external callers. Do not scatter many `feature_*.go` files flat in the parent package unless they are genuinely separate families already owned by that package.
