...
# Resolve sys.path to find workflow-runtime sibling scripts
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", "skills", "workflow-runtime", "scripts")))
...
class WorkflowCoordinator:
    ...
    def _verify_safety_gates(self, target_skill: str, target_phase: str) -> bool:
        ...
        if target_phase in ["implementation", "debug"] and target_skill in ["blueprint-to-implementation", "quick-feature", "quick-fix"]:
            ...
            if not bp.get("approved") and target_skill == "blueprint-to-implementation":
                raise GateViolationError("Approved Technical Design Blueprint required to start implementation.")
        return True
    ...
    def _verify_blueprint_gate(self, work_item_id: str) -> bool:
        ...
        approvals = store.get("approvals") or {}
        bp = approvals.get("blueprint", {})
        return bp.get("approved") is True
