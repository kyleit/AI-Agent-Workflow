...
status: active
activation_mode: specialist_on_demand
auto_route: false
orchestrator_required: true
direct_user_entry: false
test_execution_owner: TESTER
source_write_requires_task_ownership: true
...
## 0. Specialist-on-Demand Routing Contract

- **Status**: `active`
- **Activation Mode**: `specialist_on_demand`
- **Auto-Route**: `false` (MUST NOT be auto-routed by standard workflow sequence)
- **Orchestrator Dispatch**: `required`
- **Entry Conditions**: Eligible ONLY when an explicit architectural decision requires formal recording in `docs/adr/ADR-XXX_*.md` as recommended by `plan-to-blueprint` or dispatched by Architect/Orchestrator. Does NOT replace Blueprint or Architect Review.
- **Exit Behavior**: Creates dedicated ADR document under `docs/adr/ADR-XXX_short_title.md`. MUST NOT auto-route the next workflow phase or self-approve gates.
...
- **Progress Tracking**:
  - *Start*: Run `aiwf start --skill "create-adr" --command "adr" --checkpoint 3 --step "Starting execution..."`
  - *Step Updates*: Run `aiwf step --step "<step_desc>" --log "<progress_message>"` progressively during major steps.
  - *Completion*: Run `aiwf complete --checkpoint 3 --step "Step Complete" --next-skill "plan-to-blueprint" --next-command "blueprint"` when execution finishes successfully.
