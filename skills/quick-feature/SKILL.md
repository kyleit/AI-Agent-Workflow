...
- **Approval Gate Policy** (Section 1) - Do not stop for human approval during Specification or Blueprint drafting/review. Stop only at the final Blueprint Approval gate before source-code implementation.
...
- **No Premature Implementation**: No source code may be created, deleted, or modified before a Technical Design Blueprint is generated under `docs/features/<feature-family>/blueprints/` and explicitly approved by the user.
...
**Hard routing guard:** a request that initializes a repository, introduces a
backend plus frontend or database, adds a desktop shell, changes more than one
architecture boundary, or contains more than three independent acceptance
capabilities is never eligible for the lightweight Quick-Feature Blueprint.
The coordinator MUST route it to the standard `plan-to-blueprint` contract,
which derives a dynamic phase count and emits the master/phase artifact set.
If discovery expands a Quick Feature past these limits, the workflow MUST
reclassify before Blueprint generation; it must not squeeze the work into the
...
2. **For each checklist item in Section 6 of the Blueprint, follow this exact loop:**
   ```
   a. Read the checklist item (e.g., "- [ ] Thêm AutoSyncToEngine vào ProjectModel")
...
3. If implementation of an item is blocked (e.g., file not found, compile error), mark it `- [!]`
   (blocked), document the blocker inline, and continue with remaining items. Do NOT skip silently.
...
loop. It derives a feature-specific phase count from actual scope, creates the
