...
RESOLVED_PROJECT_ROOT=$(cd "$(pwd)" && pwd)
RESOLVED_SCRIPT_DIR=$(cd "$SCRIPT_DIR" && pwd)
if [ "$RESOLVED_PROJECT_ROOT" = "$RESOLVED_SCRIPT_DIR" ]; then
    log_success "Target is the framework source repository itself. Skipping update."
    exit 0
fi
...
copy_diff_item() {
    ...
    if [ ! -e "$src" ]; then
        log_warn "Source path not found in framework root, skipping: $src"
        return 0
    fi
...
if [ -d "$SCRIPT_DIR/githooks" ]; then
    copy_diff_item "$SCRIPT_DIR/githooks" "$SRC_INSTALL_TARGET/githooks"
    chmod +x "$SRC_INSTALL_TARGET/githooks/pre-commit" "$SRC_INSTALL_TARGET/githooks/pre-push" 2>/dev/null || true
fi
...
if [ -d "$SCRIPT_DIR/aiwf_release" ]; then
    copy_diff_item "$SCRIPT_DIR/aiwf_release" "$SRC_INSTALL_TARGET/aiwf_release"
fi
if [ -f "$SCRIPT_DIR/release.py" ]; then
    copy_diff_item "$SCRIPT_DIR/release.py" "$SRC_INSTALL_TARGET/release.py"
fi
if command -v git >/dev/null 2>&1 && git -C "$PROJECT_ROOT" rev-parse --is-inside-work-tree >/dev/null 2>&1 && [ -d "$SRC_INSTALL_TARGET/githooks" ]; then
    git -C "$PROJECT_ROOT" config core.hooksPath ".agents/githooks" 2>/dev/null || true
    log_success "Source-write gate wired (git core.hooksPath -> .agents/githooks)."
fi
    ...
    if [ -f "$SCRIPT_DIR/docs/release-guide.md" ]; then
        ...
        copy_diff_item "$SCRIPT_DIR/docs/features/release-public-export/docs/release-guide.md" "$SRC_INSTALL_TARGET/docs/release-guide.md"
    fi
...
for skill in $SRC_SKILLS; do
    skill_src_md="$SCRIPT_DIR/$SRC_SKILL_DIR/$skill/SKILL.md"
    if validate_skill_md "$skill_src_md" "$skill"; then
        copy_diff_item "$SCRIPT_DIR/$SRC_SKILL_DIR/$skill" "$SRC_INSTALL_TARGET/$SRC_SKILL_DIR/$skill"
    else
        log_warn "Keeping existing mirror for $skill (source has invalid SKILL.md)"
...
if [ "$SKILL_ERRORS" -gt 0 ]; then
    log_error "Update completed with warnings: $SKILL_ERRORS skill(s) skipped due to invalid SKILL.md frontmatter."
    exit 1
fi
