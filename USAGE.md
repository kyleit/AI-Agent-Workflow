...
* **Cách kích hoạt**: 
  ...
  ```
* **Ràng buộc an toàn**: Nếu thiết kế yêu cầu ADR mà tệp tin ADR tương ứng chưa được chấp thuận (Accepted), quá trình sẽ dừng lại và yêu cầu chạy `/adr`.
* **Sản phẩm đầu ra**: Mã nguồn hoàn chỉnh trong dự án của bạn.

---

### Pha 6: Phát hành tính năng (Release)
Băm phiên bản tự động, ghi log release và commit/push lên Git.
* **Cách kích hoạt**: 
  ```bash
  /release version="1.x.x"
  ```
